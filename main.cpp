...
int main()
{
    ...
    while   ( 1 ) 
    {
        ...
        sprintf( buf, "0x%X,%2.1f,%s", id, hum, cls[type] ); 
        type++;
        if  ( type > 2 )
            type = 0;     
...
// Temperatur
        ...
        sprintf( buf, "0x%X,%2.2f,%s", id, temp, cls[type] ); 
        type++;
        if  ( type > 2 )
            type = 0;     
